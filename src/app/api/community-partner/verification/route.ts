...
function validCertifiedDate(value: string){
 if(!/^\d{4}-\d{2}-\d{2}$/.test(value)) return false;
 const certified = new Date(`${value}T00:00:00Z`);
 if(Number.isNaN(certified.getTime())) return false;
 const now = new Date();
 const sixMonthsAgo = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth()-6, now.getUTCDate()));
 return certified <= now && certified >= sixMonthsAgo;
}
...
export async function POST(request:NextRequest){
 ...
 const {error}=await supabaseAdmin.from("community_partner_profiles").update({
   verification_application_role:role,
   verification_application_affiliation:affiliation||null,
   verification_identity_certifier_type:certifierType,
   verification_identity_certifier_name:certifierName,
   verification_certified_date:certifiedDate,
   verification_reference_name:referenceName,
   verification_reference_contact:referenceContact,
   verification_credentials_details:credentials||null,
   verification_safeguarding_agreed:true,
   verification_safeguarding_agreed_at:now,
   verification_status:"pending",
   verification_submitted_at:now,
   verification_reviewed_at:null,
   verification_reviewed_by:null,
   verification_identity_evidence:false,
   verification_contact_confirmed:false,
   verification_reference_reviewed:false,
   verification_credentials_reviewed:false
